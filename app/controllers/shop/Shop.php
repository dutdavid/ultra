...
class Shop extends MY_Shop_Controller
{
    ...
    public function order()
    {
        ...
        if ($guest_checkout) {
            ...
            $this->form_validation->set_rules('shipping_city', lang('shipping_address') . ' ' . lang('city'));
            ...
            if ($guest_checkout || $address = $this->shop_model->getAddressByID($this->input->post('address'))) {
                ...
                foreach ($this->cart->contents() as $item) {
                    ...
                    if ($product_details = $this->shop_model->getProductForCart($item['product_id'])) {
                        $price = $this->sma->setCustomerGroupPrice(($this->loggedIn && $product_details->special_price ? $product_details->special_price : $product_details->price), $this->customer_group);
